...
const categories = [
  ...
  { value: 'nightpant', label: 'Night Pant' },
  { value: 'childwear', label: 'Child Wear' },
  { value: 'coordsets', label: 'Co-ord Sets' },
  { value: 'nightdress', label: 'Ladies Night Dress' },
];
...
const AddProductForm: React.FC = () => {
  ...
  const validate = () => {
    ...
    if (addedColors.length === 0) newErrors.colors = 'Add at least one color.';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      alert('Product submitted! (No backend, just UI + validation)');
    }
  };
      ...
      <div className="bg-black p-6 rounded-lg border border-gray-800 space-y-6">
        ...
        <div>
          ...
          <div className="flex gap-2 items-center mb-2">
            ...
            />
            <button type="button" onClick={handleAddColor} className="bg-blue-600 text-white px-3 py-1 rounded">Add</button>
          </div>
          <div className="flex flex-wrap gap-2 mb-2">
            ...
            {addedColors.map((c, idx) => (
              <span key={idx} className="flex items-center gap-1 bg-gray-800 text-white px-2 py-1 rounded text-xs">
                ...
                {c.name}
                <button type="button" onClick={() => handleRemoveColor(idx)} className="ml-1 text-red-400">&times;</button>
              </span>
            ))}
          </div>
        ...
        <div className="pt-4">
          <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded font-semibold">Submit Product</button>
        </div>
      </div>
    </form>
  );
