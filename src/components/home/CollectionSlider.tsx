...
const CollectionSlider: React.FC<CollectionSliderProps> = ({ collectionItems }) => {
  ...
  return (
    <>
      {/* Collection Slider */}
      <div className="mb-12 overflow-hidden relative px-4 sm:px-6 md:px-8 lg:px-10">
        {/* Prev button */}
        <button
          onClick={prevSlide}
        ...
        <div className="flex justify-center mt-6 z-30 relative">
          {Array.from({ length: totalPages }).map((_, idx) => (
            <button
              ...
              className={`mx-1 w-2 h-2 md:w-3 md:h-3 rounded-full ${idx === sliderIndex ? 'bg-orange-500' : 'bg-gray-300'
                }`}
